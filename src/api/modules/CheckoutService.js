...
Service.checkoutMoMo = async (params, callback) => {
  if (!params) {
    let result = _error(1000);
    return callback(1000, { data: result });
  }
  ...
  let result = _success(200);
  result.orderData = rsOrd;
  return callback(null, result);
};

Service.callbackMoMo = async (params, callback) => {
  if (!params) {
    let result = _error(1000);
    return callback(1000, { data: result });
  }
  // console.log(params, "callbackMOMO");
  // return;

  let { user, partnerCode, order_id, amount, trans_id, resultCode, requestId } =
    params;
  ...
  if (!partnerCode || !order_id || !amount || !trans_id || !requestId) {
    let result = _error(1000);
    return callback(1000, { data: result });
  }
  if (resultCode !== "0") {
  ...
  let [errO, rsO] = await Untils.to(
    Order.findOne({ where: { id: order_id, status: 0 }, raw: true })
  );
  if (errO) {
    let result = _error(8201, errO);
  ...
  let [errOU, rsOU] = await Untils.to(
    Order.update(dataUpdate, { where: { id: order_id, status: 0 }, raw: true })
  );
  if (errOU) {
    console.log(errOU);
    let result = _error(8800, errOU);
    return callback(8800, { data: result });
  }

  mailer.sendMail(
    rsO.user_checkout.email,
    "Checkout Success",
    `
      <!DOCTYPE HTML PUBLIC "-//W3C//DTD XHTML 1.0 Transitional //EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
  <html xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
