...
Service.useVoucher = async (params, callback) => {
  ...
  [errVoucher, rsVoucher] = await Untils.to(
    Voucher.update(
      {
        is_active: 1,
      },
...
Service.getVoucherForUser = async (params, callback) => {
  if (!params) {
    let result = _error(1000);
    return callback(1000, { data: result });
  }
  ...
  let errV, rsV;
  [errV, rsV] = await Untils.to(Voucher.findAll(findVoucher));
  if (errV) {
    let result = _error(4001, errV);
    return callback(4001, { data: result });
  }
  if (!rsV) {
    let result = _error(4001);
